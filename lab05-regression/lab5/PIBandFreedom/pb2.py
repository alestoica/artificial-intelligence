...
def splitDataIntoTrainingAndValidationSubsets(inputs, outputs):
    ...
    # trainSample = indexes[:train_size]
    # validationSample = indexes[train_size:]
...
def trainingManually(trainInputs, trainOutputs):
    ...
    regressor = MyLinearBivariateRegression()
    # regressor.fit([point[0] for point in trainInputs], [point[1] for point in trainInputs], trainOutputs)
...
def predictNewInputs(regressor, validationInputs, validationOutputs):
    # computedValidationOutputs = regressor.predict([point[0] for point in validationInputs], [point[1] for point in validationInputs])
...
def main():
    filePath = '../data/v1_world-happiness-report-2017.csv'
    ...
    regressor, w0, w1, w2 = trainingManually(trainInputs, trainOutputs)
